...
AddEventHandler('esx:playerLoaded', function(xPlayer)
	...
	ESX.TriggerServerCallback('esx_lscustom:getVehiclesPricesImported', function(vehicles)
...
Citizen.CreateThread(function()
	while true do
		...
		if IsPedInAnyVehicle(playerPed, false) then
			...
			if (PlayerData.job and PlayerData.job.name == 'tunners') or not Config.IsMechanicJobOnly then
				for k,v in pairs(Config.Zones) do
			...
			if IsControlJustReleased(0, 38) and not lsMenuIsShowed and isInLSMarker then
				if (PlayerData.job and PlayerData.job.name == 'tunners') or not Config.IsMechanicJobOnly then --- Mudar Aqui o Job Name do Emprego.
					lsMenuIsShowed = true
